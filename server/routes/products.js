...
router.post('/', async (req, res) => {
  try {
    ...
    if (price === undefined || isNaN(Number(price))) return res.status(400).json({ error: 'price (number) is required.' });

    // Auto-generate a numeric string ID higher than the current max
    ...
    res.status(201).json({ id: product._id, ...product.toObject(), unit: unit ?? 'packs' });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
